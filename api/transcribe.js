...
export default function App() {
  ...
  async function routeIntent(heard) {
    ...
    let intent = "unclear";
    if (lower.includes("story") || lower.includes("tell you") || lower.includes("something happened")) intent = "story";
    else if (lower.includes("question") || lower.includes("ask")) intent = "question";
    else if (lower.trim().length > 3) {
      try {
        const data = await callClaude({
          model: "claude-sonnet-4-6",
          max_tokens: 50,
          messages: [{ role: "user", content: `Marty was asked "Do you have a story for me, or should I ask you a question?" He replied: "${heard}". Does he want to TELL a story, or be ASKED a question? Reply with exactly one word: STORY, QUESTION, or UNCLEAR.` }]
        });
    ...
    if (intent === "story") {
      ...
      setCurrentQuestion("No rush, Marty — tap a button below whenever you're ready.");
      setCurrentChapter("Hello");
